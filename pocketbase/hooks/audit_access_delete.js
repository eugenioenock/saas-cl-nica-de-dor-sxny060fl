onRecordAfterDeleteSuccess((e) => {
  ...
  try {
    ...
    console.log(`[AUDIT] Access revoked: User ${userEmail} from Clinic ${clinicName}`)
    ...
    if (apiKey) {
      $http.send({
        ...
        body: JSON.stringify({
          ...
          to: toEmail,
          subject: `Security Alert: Access Revoked - ${clinicName}`,
          html: `<p>Security Notification</p><p>User <b>${userEmail}</b> had their access revoked from the clinic <b>${clinicName}</b>.</p>`,
        }),
      })
    }
  } catch (err) {
    console.log('Audit hook delete error:', err)
  }
  e.next()
}, 'user_clinic_access')
